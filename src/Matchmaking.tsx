...
import { Swords, Search, Loader2, X, Trophy, Users, UserPlus, Trash2, Circle } from 'lucide-react';
...
export const Matchmaking: React.FC<MatchmakingProps> = ({ user, onMatchFound }) => {
    ...
    useEffect(() => {
        ...
        lobbyChannel
            ...
            .on('presence', { event: 'join' }, ({ newPresences }) => {
                // Potential toast: New player joined the lobby
            })
    ...
    return (
        <div className="flex flex-col lg:flex-row items-center lg:items-start justify-center h-full p-4 lg:p-8 gap-8 overflow-y-auto">
            ...
            <div className="w-full lg:w-80 bg-white/5 backdrop-blur-md border border-white/10 rounded-3xl p-6 flex flex-col h-[500px] lg:h-full max-h-[600px]">
                ...
                <div className="relative mb-6">
                    <input
                        ...
                        className="w-full bg-black/40 border border-white/10 rounded-xl py-3 pl-4 pr-12 text-sm text-white placeholder:text-slate-600 focus:outline-none focus:ring-2 focus:ring-purple-500/50 transition-all"
                ...
                <div className="flex-1 overflow-y-auto space-y-3 pr-2 scrollbar-thin scrollbar-thumb-white/10">
                    {friends.length === 0 ? (
                        <div className="text-center py-12">
                            <Users size={32} className="text-slate-700 mx-auto mb-3 opacity-20" />
                            <p className="text-slate-600 text-sm font-medium">No friends yet.<br />Start building your alliance!</p>
                        </div>
                    ) : (
                        friends.map((friend) => (
